...
function isInsideCircle(circle, point) {
  ...
  return xLine + yLine < circle.radius ** 2;
}
...
function getDigitalRoot(num) {
  ...
  const sumOfDigits = arrayOfDigits.reduce((acc, value) => acc + Number(value), 0);
  return sumOfDigits > 9 ? getDigitalRoot(sumOfDigits) : sumOfDigits;
}
...
function isBracketsBalanced(str) {
  ...
  while (true) {
    ...
    strCopy = strCopy.replace('{}', '');
    strCopy = strCopy.replace('<>', '');
    if (!strCopy.includes('[]') && !strCopy.includes('()') && !strCopy.includes('{}') && !strCopy.includes('<>')) {
      break;
    }
  }
